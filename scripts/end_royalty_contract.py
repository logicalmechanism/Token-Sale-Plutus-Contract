...
def end(tmp, wallet_skey_path, wallet_addr, script_addr, cost, royalty, datum_hash, plutus_script,seller_addr,royalty_addr, policy_id, token_name, collateral):
    """
    Retrieves a token from a royalty smart contract
    """
...
if __name__ == "__main__":
    ...
    # ROYALTY_ADDR = trx.get_address_from_vkey("/path/to/royalty/payment.vkey")
    ROYALTY_ADDR = "ROYALTY_ADDRESS_HERE"

    # Script info
    PLUTUS_SCRIPT_PATH = "/path/to/compiled/contract/plutus.script"
    # SCRIPT_ADDR   = trx.get_script_address(PLUTUS_SCRIPT_PATH)
    SCRIPT_ADDR   = "SCRIPT_ADDERSS_HERE"
    ...
    end(TMP, BUYER_SKEY_PATH, BUYER_ADDR, SCRIPT_ADDR, COST, ROYALTY_COST, DATUM_HASH, PLUTUS_SCRIPT_PATH, SELLER_ADDR, ROYALTY_ADDR, POLICY_ID, TOKEN_NAME, COLLATERAL)